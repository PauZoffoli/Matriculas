...
@php
        ...
        //$valor = floor(floor(floor($arancelAnual / 11 * $maximoDeCuotas) / $contrato->nroCuotas) * (1 - //$contrato -> PorcentajeBeca / 100)) + (floor((((floor($arancelAnual / 11 ) * $maximoDeCuotas) % //$contrato->nroCuotas)) * (1-$contrato->PorcentajeBeca / 100)));
  
  //$valor = $valor + $valor % $contrato->nroCuotas;
        
        
        $valor = (floor(floor($arancelReajustadoConBeca / 11 * $maximoDeCuotas) / $contrato->nroCuotas)) + (floor(($arancelReajustadoConBeca/11) * $maximoDeCuotas) % ($contrato->nroCuotas));
...
$startDate = '07-'. sprintf("%02d", $mes) . '-' . $contrato->anioAContratar ;
...
@if ($mes != 2)
  ...
  border-top:none;padding:0cm 3.5pt 0cm 3.5pt;height:6.3pt'>
  <p class=MsoNormal align=center style='text-align:center'><span lang=ES-CL
  style='font-size:10.0pt;font-family:"Arial",sans-serif;color:black'>Cuota {{ $cuota }}</span></p>
  </td>
  <td width=80 nowrap valign=bottom style='width:60.05pt;border-top:none;
  border-left:none;border-bottom:solid windowtext 1.0pt;border-right:solid windowtext 1.0pt;
  padding:0cm 3.5pt 0cm 3.5pt;height:6.3pt'>
