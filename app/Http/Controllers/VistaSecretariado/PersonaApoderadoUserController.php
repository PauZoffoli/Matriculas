...
class PersonaApoderadoUserController extends AppBaseController
{
    ...
    public function __construct(ApoderadoRepository $apoderadoRepo, PersonaRepository $personaRepo, UserRepository $userRepo, TipoPersonaRepository $tpRepo, UserRolRepository $usRol)
    {
    ...
    public function agregarPersonaApoderado(request $request)
    {
        ...
        // en primer lugar ingreso al usuario, para despues asignar este id a persona
       
        //eliminamos los dos ultimos caracteres del rut, el guion más el último dígito.
         $rutParaUsuario = $request->rut;
         $num = 2;
         $user = substr($rutParaUsuario, 0, -$num);
         //fin

         $passwordFormateada = Helper::deleteSpacesUpperText($request->ApPat); 
         ...
         $usuario = $this->userRepository->create(['name' => $user, 'email' => $request->email, 'password' => $passwordFormateada]);
       
         //en segundo lugar ingreso a la persona
         $request['idUser'] = $usuario->id;  
         $persona = $this->personaRepository->create($request->all());
          ...
          $apoderado = $this->apoderadoRepository->create(['idPersona' => $persona->id, 'estado' => "MatriculaNoRevisadaPorApoderado"]); 
          //fin

          //creación de TipoPersona para asignarle un rol al apoderado, apoderado nuevo o antiguo
          $tipo = $this->tipoPersonaRepository->create(['idTipo' => $request->tipoApoderado,'idPersona' => $persona->id]);

          //creación user rol
           $tipo = $this->userRolRepository->create(['idRol' => $request->userRol,'idUser' => $usuario->id]);

          Flash::success('La persona ' . $request->PNombre . ' ' . $request->ApPat . ' Rut: ' . $request->rut .  ', ha sido creada exitósamente.');
          return redirect(route('cambioApoderado'));
